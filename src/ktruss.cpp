...
void base_ktruss (Graph& graph, bool hierarchy, edge nEdge, vector<vertex>& K, vector<vertex>& KK, vertex* maxtruss, string vfile, FILE* fp) {
...
//    FILE* fffp = fopen ("no_of_tri", "w");
//    for (size_t i = 0; i < orderedGraph.size(); i++)
//        for (size_t j = 0; j < orderedGraph[i].size(); j++) {
//            int v = orderedGraph[i][j];
//            fprintf (fffp, "%d \t %d \t %d \t %d \t %d\n", i, j, v, KK[i], TC[i][j]);
//        }
//    fclose(fffp);
    
    string vfile1 = "P3_" + vfile;
    FILE* P3 = fopen (vfile1.c_str(), "w");
    for (size_t i = 0; i < graph.size(); i++){
        ...
        for (size_t j = 0; j < graph[i].size(); j++) {
            ...
            TRI.push_back(TriC[f]);
            TRU.push_back(K[f]);
        }
        if ((!TRI.empty()) && (!TRU.empty())) {
            double sum = accumulate(TRU.begin(), TRU.end(), 0.0);
            double mean = sum / TRU.size();
            double sq_sum = inner_product(TRU.begin(), TRU.end(), TRU.begin(), 0.0);
            double stdev = sqrt(sq_sum / TRU.size() - mean * mean);
            int max = *max_element(TRU.begin(),TRU.end());
            int min = *min_element(TRU.begin(),TRU.end());
            
            double sum1 = accumulate(TRI.begin(), TRI.end(), 0.0);
            double mean1 = sum1 / TRI.size();
            double sq_sum1 = inner_product(TRI.begin(), TRI.end(), TRI.begin(), 0.0);
            double stdev1 = sqrt(sq_sum1 / TRI.size() - mean1 * mean1);
            int max1 = *max_element(TRI.begin(),TRI.end());
            int min1 = *min_element(TRI.begin(),TRI.end());
            fprintf (P3, "%d \t %d\t %d \t %lf \t %lf \t %d \t %d \t %lf \t %lf \t %d \t %d\n",i, KK[i], graph[i].size(), mean, stdev, min, max, mean1, stdev1, min1, max1);
        }
        
    }
    fclose(P3);
    
    string vfile2 = "P2_" + vfile;
    FILE* P2 = fopen (vfile2.c_str(), "w");
    int edg = 0;
    for (size_t i = 0; i < orderedGraph.size(); i++)
        for (size_t j = 0; j < orderedGraph[i].size(); j++) {
            int v = orderedGraph[i][j];
            fprintf (P2, "%d \t %d \t %d \t %d \t %d \t %d \t %d \t %d \t \n", i, v, K[edg], TC[i][j], graph[i].size(), graph[v].size(), KK[i], KK[v]);
            edg++;
        }
    fclose(P2);
